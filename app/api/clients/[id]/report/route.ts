...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  const policies = row.policies.map((policy) => ({
    ...
    ongoingInvestmentStartDate: policy.ongoingInvestmentStartDate?.toISOString().slice(0, 10),
    ongoingInvestmentEndDate: policy.ongoingInvestmentEndDate?.toISOString().slice(0, 10),
