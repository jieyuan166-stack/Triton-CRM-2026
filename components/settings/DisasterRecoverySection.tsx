    ...
    <section className="mb-6 overflow-hidden rounded-xl border border-amber-200 bg-amber-50/30 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-amber-100 px-5 py-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-600">Encrypted full CRM backup: database, relationships, users, settings, and managed uploads. Verified archives are stored in the configured private offsite backup provider.</p>
          {latest ? <p className="mt-2 text-xs text-slate-500">Last verified: {formatDate(latest.verifiedAt)} · {latest.remote.uploaded ? "Offsite uploaded" : "Offsite pending"} · {latest.email.sent ? "email sent" : "email pending"}</p> : null}
        </div>
      ...
      {loading ? <div className="flex h-24 items-center justify-center text-sm text-slate-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Loading encrypted backups…</div> : backups.length === 0 ? <div className="px-5 py-6 text-sm text-slate-600">No completed encrypted disaster-recovery backup yet. Configure the NAS backup secrets and private offsite provider, then run a full backup.</div> : (
        <>
      ...
      <Dialog open={deleteTargets.length > 0} onOpenChange={(open) => { if (!open) { setDeleteTargets([]); setDeleteConfirm(""); } }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Delete {deleteTargets.length === 1 ? "encrypted backup" : `${deleteTargets.length} encrypted backups`}?</DialogTitle><DialogDescription>This permanently removes each selected encrypted archive, checksum, recovery metadata, and its configured offsite copy.</DialogDescription></DialogHeader>
