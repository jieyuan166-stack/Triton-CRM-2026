...
try {
  ...
  await annualDialog.getByLabel(/Summary/).fill("Annual reset review");
  await annualDialog.getByLabel("Deadline").fill("2026-12-15");
  await annualDialog.getByLabel("Repeat annually").check();
  await annualDialog.getByLabel("Email me one month before").check();
  await annualDialog.getByRole("button", { name: /Save Follow-up/i }).click();
  await annualDialog.waitFor({ state: "hidden" });
  const annualRow = page.getByText("Annual reset review", { exact: true }).first().locator("xpath=ancestor::li");
  await annualRow.getByRole("button", { name: "Mark Done" }).click();
  await page.getByText("Follow-up marked done", { exact: true }).waitFor();
  const afterAnnualDone = await (await context.request.get(`${base}/api/data`)).json();
  const annualRows = afterAnnualDone.followUps.filter((followUp) => followUp.summary === "Annual reset review");
  assert.equal(annualRows.length, 2);
  assert.ok(annualRows.some((followUp) => followUp.completedAt));
  assert.ok(annualRows.some((followUp) => !followUp.completedAt && followUp.deadline === "2027-12-15"));
